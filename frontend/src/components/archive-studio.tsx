...
export function ArchiveFolioDialog({ project, cards, fieldworkLabel = "再次采风", busy, onClose, onSave, onDeleteRequest, onRestartFieldwork }: { project: Project; cards: ArchiveCard[]; fieldworkLabel?: string; busy: boolean; onClose: () => void; onSave: (card: ArchiveCard) => Promise<boolean>; onDeleteRequest: (card: ArchiveCard) => void; onRestartFieldwork?: () => void }) {
  ...
  const editable = draft ?? selected;
  function selectCard(card: ArchiveCard) { setSelectedId(card.id); setDraft(null); }
