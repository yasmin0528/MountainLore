...
export function BrandManualResult({ workspace, logoTask, patternTask, exportTask, busy, onGenerate, onSelect, onSave, onRefresh, onRetry, onGenerateAsset, onFailure, onOpenArchive, onNext }: { workspace: Workspace; logoTask?: WorkflowTask; patternTask?: WorkflowTask; exportTask?: WorkflowTask; busy: boolean; onGenerate: (preferences: ManualVisualPreferences) => Promise<void>; onSelect: (id: string) => Promise<boolean>; onSave: (content: Content) => Promise<void>; onRefresh: () => Promise<void>; onRetry: (id: string) => void; onGenerateAsset: (kind: "extension_pattern" | "packaging_key_visual") => Promise<void>; onFailure: (message: string) => void; onOpenArchive: () => void; onNext: () => void }) {
