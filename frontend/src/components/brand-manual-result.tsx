...
function buildDraft(workspace: Workspace): Content {
  ...
  return { ...source, display_name: text(source.display_name, workspace.project.brand_name), cover_subtitle: text(source.cover_subtitle, "品牌手册"), logo_note: text(source.logo_note), logo_design: text(source.logo_design, text((source.visual_system as Content | undefined)?.logo_direction)), font_family: text(source.font_family, FONT_OPTIONS[0].value), font_label: text(source.font_label, FONT_OPTIONS[0].label), color_palette: [...palette, ...DEFAULT_PALETTE.slice(palette.length)].slice(0, 4), brand_one_liner: text(source.brand_one_liner, text(source.slogan)), slogan: text(source.slogan, text(source.brand_one_liner)), target_audience: text(source.target_audience, text(strategy.audience)), target_scenarios: text(source.target_scenarios, Array.isArray(strategy.scenarios) ? strategy.scenarios.join("、") : text(strategy.scenarios)), story_spine: text(source.story_spine, text(story.main_story, text(source.brand_introduction))), selling_points: pointList(source.selling_points) };
}
