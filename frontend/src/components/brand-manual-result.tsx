...
function pointList(value: unknown): SellingPoint[] {
  ...
  const points = raw.slice(0, 3).map((item, index) => {
    const point = typeof item === "object" && item ? item as Content : { text: String(item) };
    return { category: point.category === "创新活动策划" ? "创新活动策划" : "产品创新", explanation: text(point.explanation, text(point.text, `待完善卖点 ${index + 1}`)), text: text(point.text), claimIds: list(point.claimIds) } as SellingPoint;
  });
  while (points.length < 3) points.push({ category: points.length === 2 ? "创新活动策划" : "产品创新", explanation: `待完善卖点 ${points.length + 1}`, claimIds: [] });
  return points;
}
...
function buildDraft(workspace: Workspace): Content {
  ...
  return { ...source, display_name: text(source.display_name, workspace.project.brand_name), cover_subtitle: text(source.cover_subtitle, "品牌手册"), logo_note: text(source.logo_note, "主标志用于品牌识别与对外呈现，四周应保留足够留白。"), logo_design: text(source.logo_design, text((source.visual_system as Content | undefined)?.logo_direction, "待补充 Logo 设计说明。")), font_family: text(source.font_family, FONT_OPTIONS[0].value), font_label: text(source.font_label, FONT_OPTIONS[0].label), color_palette: [...palette, ...DEFAULT_PALETTE.slice(palette.length)].slice(0, 4), brand_one_liner: text(source.brand_one_liner, text(source.slogan, "一句话说清品牌的来处与价值。")), slogan: text(source.slogan, text(source.brand_one_liner, "把有来处的风物，带进今天。")), target_audience: text(source.target_audience, text(strategy.audience, "待确认目标人群")), target_scenarios: text(source.target_scenarios, Array.isArray(strategy.scenarios) ? strategy.scenarios.join("、") : text(strategy.scenarios, "待确认核心场景")), story_spine: text(source.story_spine, text(story.main_story, text(source.brand_introduction, "从真实档案中提炼一句话品牌故事。"))), selling_points: pointList(source.selling_points) };
}
...
function Setup({ workspace, busy, onGenerate, onFailure }: { workspace: Workspace; busy: boolean; onGenerate: (preferences: ManualVisualPreferences) => Promise<void>; onFailure: (message: string) => void }) {
  ...
  const [font, setFont] = useState(FONT_OPTIONS[0]); const [palette, setPalette] = useState(DEFAULT_PALETTE);
  ...
  return <section className="manual-onboarding-page"><header><div><p className="eyebrow">品牌档案 / 首次打开品牌手册</p><h1>先确定视觉起点，再让 AI 提三条路线。</h1><p>系统会自动读取当前品牌的 {workspace.archive_cards.filter((card) => card.status === "active").length} 张已确认档案卡；这里仅补充 Logo、字体与颜色。</p></div><img src="/guipin/assets/sticker-custom.png" alt="" /></header><ol className="manual-setup-steps">
    <li><article><span>01</span><div><h2>Logo</h2><p>上传已有 Logo，或先比较三条文字设计方案；确定 current 后才调用生图。</p></div></article><div className="manual-choice-row"><button className={logoMode === "upload" ? "is-selected" : ""} onClick={() => setLogoMode("upload")}>上传 Logo</button><button className={logoMode === "ai" ? "is-selected" : ""} onClick={() => setLogoMode("ai")}>比较文字方案</button></div>{logoMode === "upload" ? <label className="manual-logo-drop">{logoPreview ? <img src={logoPreview} alt="上传 Logo 预览" /> : <span>选择 PNG、JPG 或 WebP</span>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={uploadLogo} /></label> : <div className="manual-ai-logo-note"><b>暂不调用生图模型</b><p>三条路线会各自给出 Logo 的符号、构图与色彩说明；选择一条后才生成无文字 Logo，并保存到品牌资产。</p></div>}{notice && <small className="manual-setup-notice">{notice}</small>}</li>
    <li><article><span>02</span><div><h2>字体</h2><p>选择手册标题与正文的基础气质。</p></div></article><div className="manual-font-options">{FONT_OPTIONS.map((option) => <button key={option.value} className={font.value === option.value ? "is-selected" : ""} style={{ fontFamily: option.value }} onClick={() => setFont(option)}>{option.label}</button>)}</div></li>
    <li><article><span>03</span><div><h2>颜色</h2><p>{logoAssetId ? "以下四色来自 Logo，可继续调整。" : "上传 Logo 后自动提取四色；AI 模式先使用缺省色。"}</p></div></article><div className="manual-palette-fields">{palette.map((color, index) => <label key={`${index}-${color}`}><input type="color" value={color} onChange={(event) => setPalette((items) => items.map((item, itemIndex) => itemIndex === index ? event.target.value.toUpperCase() : item))} /><input value={color} maxLength={7} onChange={(event) => setPalette((items) => items.map((item, itemIndex) => itemIndex === index ? event.target.value.toUpperCase() : item))} aria-label={`候选色 ${index + 1}`} /></label>)}</div></li>
  </ol><footer><p>下一步将并排生成三条受众、场景、故事与视觉均有清晰差异的路线。</p><button className="primary-button" disabled={busy || uploading || !canGenerate} onClick={() => onGenerate({ logo_mode: logoMode, logo_media_asset_id: logoAssetId || undefined, font_family: font.value, font_label: font.label, palette })}>{uploading ? "正在读取 Logo…" : busy ? "正在生成…" : "生成三条品牌路线 →"}</button></footer></section>;
}
...
function RouteCompare({ routes, busy, onSelect, onRegenerate }: { routes: Direction[]; busy: boolean; onSelect: (id: string) => Promise<boolean>; onRegenerate: () => void }) {
  ...
  return <section className="manual-route-page"><header><p className="eyebrow">品牌手册 / 路线比较</p><h1>三条路线，选择一条成为 current。</h1><p>同一组已确认档案，在核心人群、场景、叙事、视觉语气与 Logo 设计方案上刻意拉开差异。</p></header><div className="manual-route-compare">{routes.map((route, index) => { const content = routeContent(route); return <article key={route.id} className={selected === route.id ? "is-selected" : ""}><span>ROUTE {String(index + 1).padStart(2, "0")}</span><h2>{route.title.replace(/^路线[一二三]｜/, "")}</h2><p className="manual-route-line">{text(content.brand_one_liner, "待生成品牌一句话")}</p><dl><div><dt>目标人群</dt><dd>{text(content.target_audience, "待确认")}</dd></div><div><dt>核心场景</dt><dd>{list(content.target_scenarios).join(" · ") || text(content.target_scenarios, "待确认")}</dd></div><div><dt>故事主线</dt><dd>{text(content.story_spine, "待确认")}</dd></div><div className="manual-logo-concept"><dt>Logo 设计方案</dt><dd>{text(content.logo_design, "待生成 Logo 设计说明")}</dd></div></dl><ul>{pointList(content.selling_points).map((point, pointIndex) => <li key={pointIndex}><small>{point.category}</small>{point.explanation}</li>)}</ul><div className="manual-route-keywords">{list(content.visual_keywords).slice(0, 4).map((word) => <i key={word}>{word}</i>)}</div><button className="primary-button" disabled={busy} onClick={async () => { setSelected(route.id); await onSelect(route.id); }}>选择这条路线</button></article>; })}</div><footer><button className="text-button" disabled={busy} onClick={onRegenerate}>三条都不合适，重新生成</button><span>确认路线后才调用生图模型，并将最终 Logo 保存到品牌资产。</span></footer></section>;
}
...
export function BrandManualResult({ workspace, manualTask, logoTask, patternTask, exportTask, demoMode, busy, onGenerate, onSelect, onSave, onRefresh, onRetry, onGenerateAsset, onFailure, onOpenArchive, onNext }: { workspace: Workspace; manualTask?: WorkflowTask; logoTask?: WorkflowTask; patternTask?: WorkflowTask; exportTask?: WorkflowTask; demoMode: boolean; busy: boolean; onGenerate: (preferences: ManualVisualPreferences) => Promise<void>; onSelect: (id: string) => Promise<boolean>; onSave: (content: Content) => Promise<void>; onRefresh: () => Promise<void>; onRetry: (id: string) => void; onGenerateAsset: (kind: "extension_pattern" | "packaging_key_visual") => Promise<void>; onFailure: (message: string) => void; onOpenArchive: () => void; onNext: () => void }) {
  ...
  const editableText = (key: string, multiline = true) => editing ? (multiline ? <textarea value={text(draft[key])} onChange={(event) => setField(key, event.target.value)} /> : <input value={text(draft[key])} onChange={(event) => setField(key, event.target.value)} />) : <p>{text(draft[key], "待完善")}</p>;

  return <section className="manual-deck-page" tabIndex={0} onKeyDown={handleKeys} aria-label={`${workspace.project.brand_name} 品牌手册`}><header className="manual-deck-top"><button className="text-button" onClick={onOpenArchive}>← 品牌档案</button><div><span className="manual-current-tag">current</span><b>{current?.title ?? "当前品牌路线"}</b></div></header><div className="manual-deck-layout">
    <article className="manual-slide-canvas" style={{ "--manual-primary": palette[0] ?? DEFAULT_PALETTE[0], "--manual-secondary": palette[1] ?? DEFAULT_PALETTE[1] } as CSSProperties}>
      {slide === 0 && <section className="manual-slide cover-slide"><div><span>BRAND GUIDELINES</span>{editing ? <input className="manual-cover-name-input" value={text(draft.display_name)} onChange={(event) => setField("display_name", event.target.value)} /> : <h1>{text(draft.display_name, workspace.project.brand_name)}</h1>}{editableText("cover_subtitle", false)}</div><Logo asset={logo} brandName={workspace.project.brand_name} /></section>}
      {slide === 1 && <section className="manual-slide logo-slide"><header><span>01</span><h1>Logo / 延展纹样</h1></header><div className="manual-logo-layout"><section className="manual-visual-asset"><small>最终 Logo</small><Logo asset={logo} brandName={workspace.project.brand_name} />{logoTask?.status === "running" || logoTask?.status === "queued" ? <em>正在生成最终 Logo · {logoTask.progress}%</em> : logoTask?.status === "failed" ? <button className="text-button" onClick={() => onRetry(logoTask.id)}>Logo 生成失败，重试</button> : logo ? <em>已沉淀至品牌资产</em> : <em>等待最终 Logo</em>}</section><section className="manual-visual-asset"><small>延展纹样</small><Pattern asset={pattern} />{patternTask?.status === "running" || patternTask?.status === "queued" ? <em>正在生成纹样 · {patternTask.progress}%</em> : patternTask?.status === "failed" ? <button className="text-button" onClick={() => onRetry(patternTask.id)}>纹样生成失败，重试</button> : pattern ? <em>已保存为品牌资产</em> : !demoMode && <button className="text-button" onClick={() => void onGenerateAsset("extension_pattern")}>生成延展纹样</button>}</section><section className="manual-logo-plan"><small>已选定的设计方案</small>{editing ? <><textarea value={text(draft.logo_design)} onChange={(event) => setField("logo_design", event.target.value)} /><textarea value={text(draft.logo_note)} onChange={(event) => setField("logo_note", event.target.value)} /><label className="manual-replace-logo">替换 Logo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={replaceLogo} /></label></> : <><p>{text(draft.logo_design)}</p><small>{text(draft.logo_note)}</small></>}</section></div></section>}
      {slide === 2 && <section className="manual-slide system-slide"><header><span>02</span><h1>字体 / 颜色</h1></header><div className="manual-system-grid"><section><small>字体方案</small>{editing ? <select value={text(draft.font_family)} onChange={(event) => { const option = FONT_OPTIONS.find((item) => item.value === event.target.value) ?? FONT_OPTIONS[0]; setDraft((value) => ({ ...value, font_family: option.value, font_label: option.label })); }}>{FONT_OPTIONS.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select> : <h2 style={{ fontFamily: text(draft.font_family) }}>{text(draft.font_label)}</h2>}<p>山 Aa 12 · 文字的来处与呼吸</p></section><section><small>品牌四色</small><div className="manual-slide-swatches">{palette.map((color, index) => <label key={`${color}-${index}`}><i style={{ backgroundColor: color }} />{editing ? <input value={color} onChange={(event) => { const next = [...palette]; next[index] = event.target.value.toUpperCase(); setField("color_palette", next); }} /> : <b>{color}</b>}</label>)}</div></section></div></section>}
