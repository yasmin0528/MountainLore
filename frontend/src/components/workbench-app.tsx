...
export default function WorkbenchApp({ initialDemo = false, initialScreen = "archive", initialManual = false, initialDirectionDraft = false, initialFieldwork = false }: { initialDemo?: boolean; initialScreen?: Screen; initialManual?: boolean; initialDirectionDraft?: boolean; initialFieldwork?: boolean }) {
  ...
  const fieldworkLabel = session && session.status === "active" && !session.ready_to_finish ? "继续采风" : "再次采风";
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><header className="mobile-workspace-bar"><button ref={mobileNavTriggerRef} type="button" className="mobile-project-mark" aria-label={mobileNavOpen ? "收起导航菜单" : "打开导航菜单"} aria-expanded={mobileNavOpen} aria-controls="mobile-workspace-navigation" onClick={() => mobileNavOpen ? closeMobileNav() : setMobileNavOpen(true)}>贵</button><div className="mobile-workspace-title"><strong>贵品风物志</strong><small>{mobileScreenTitles[screen]}</small></div></header>{demoMode && <aside className="demo-banner" role="status"><span>{demoReason ? `真实后端暂不可用（${demoReason}），已载入演示数据。` : "演示数据模式：档案、观潮来源与出山结果均为模拟内容，仅供检查页面和交互。"}</span><button className="text-button" onClick={() => window.location.reload()}>重试真实服务</button></aside>}
      ...
      {screen === "interview" && project && session && <Interview project={project} session={session} answer={answer} setAnswer={setAnswer} uploads={uploads} busy={busy} onFiles={uploadFiles} onSend={sendMessage} onFinish={finishFieldwork} onContinueFieldwork={restartFieldwork} />}
      ...
      {archiveModal === "cards" && workspace && <ArchiveFolioDialog project={workspace.project} cards={workspace.archive_cards} fieldworkLabel={fieldworkLabel} busy={busy} onClose={() => setArchiveModal(null)} onSave={saveCard} onDeleteRequest={(card) => setDeleteCardTarget(card)} onRestartFieldwork={() => { setArchiveModal(null); void restartFieldwork(); }} />}
...
function Interview({ project, session, answer, setAnswer, uploads, busy, onFiles, onSend, onFinish, onContinueFieldwork }: { project: Project; session: Session; answer: string; setAnswer: (value: string) => void; uploads: Array<{ id: string; file: File; status: string; error?: string }>; busy: boolean; onFiles: (event: ChangeEvent<HTMLInputElement>) => void; onSend: (skip?: boolean) => void; onFinish: () => void; onContinueFieldwork: () => void }) {
...
function stageToScreen(stage?: string, status?: string, hasSession?: boolean): Screen { if (stage === "positioning") return "manual"; if (stage === "chronicle" && status === "directions_ready") return "directions"; if (stage === "chronicle") return "chronicle"; if (stage === "tide") return "tide"; if (stage === "launch") return "launch"; if (stage === "fieldwork") return hasSession ? "interview" : "setup"; return "archive"; }
