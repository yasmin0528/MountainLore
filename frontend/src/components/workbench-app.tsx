...
export default function WorkbenchApp({ initialDemo = false, initialScreen = "archive", initialManual = false, initialDirectionDraft = false }: { initialDemo?: boolean; initialScreen?: Screen; initialManual?: boolean; initialDirectionDraft?: boolean }) {
  ...
  async function previewLaunch() {
    ...
    const requestPrompt = launchType === "peripheral" ? `${launchPrompt.trim()}\n\n本次需生成的实体物料：${selectedLaunchMaterials.map((item) => item.label).join("、")}。请围绕这些物料给出统一的概念方向。` : launchPrompt.trim();
    setBusy(true); setError(null);
    try {
      if (demoMode) {
      ...
      const response = await api<{ data: GenerationPreview }>(`/projects/${project.id}/generation-previews`, { method: "POST", body: JSON.stringify({ template_type: launchType, inspiration_text: requestPrompt, inspiration_card_id: launchInspiration?.id }) });
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">{demoMode && <aside className="demo-banner" role="status"><span>{demoReason ? `真实后端暂不可用（${demoReason}），已载入演示数据。` : "演示数据模式：档案、观潮来源与出山结果均为模拟内容，仅供检查页面和交互。"}</span><button className="text-button" onClick={() => window.location.reload()}>重试真实服务</button></aside>}
      ...
      {screen === "manual" && workspace && <BrandManualResult key={workspace.manual?.current_version_id ?? workspace.project.status ?? "manual-setup"} workspace={workspace} manualTask={latestManualTask} logoTask={latestLogoTask} patternTask={latestPatternTask} exportTask={workspace.tasks?.find((item) => item.kind === "export")} demoMode={demoMode} busy={busy} onGenerate={createDirections} onSelect={selectDirection} onSave={saveManual} onRefresh={refreshWorkspace} onRetry={retryTask} onGenerateAsset={generateManualAsset} onFailure={setError} onOpenArchive={() => setScreen("archive")} onNext={() => setScreen("tide")} />}
