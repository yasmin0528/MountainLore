...
export default function WorkbenchApp({ initialDemo = false, initialScreen = "archive", initialManual = false, initialDirectionDraft = false }: { initialDemo?: boolean; initialScreen?: Screen; initialManual?: boolean; initialDirectionDraft?: boolean }) {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">{demoMode && <aside className="demo-banner" role="status"><span>{demoReason ? `真实后端暂不可用（${demoReason}），已载入演示数据。` : screen === "tide" ? "演示项目中，观潮读取只读的实时联网样例；档案与出山内容仍为模拟数据。" : "演示数据模式：档案与出山结果均为模拟内容；观潮页可读取只读的实时联网样例。"}</span><button className="text-button" onClick={() => window.location.reload()}>重试真实服务</button></aside>}{error && <p className="form-error global-error" role="alert">{error}</p>}
