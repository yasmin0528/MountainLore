...
export default function FieldworkApp() {
  ...
  async function uploadFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    const items = selected.slice(0, available).map((file) => ({ id: crypto.randomUUID(), name: file.name, file, preview: URL.createObjectURL(file), status: "uploading" as const }));
    setUploads((current) => [...current, ...items]);
    await Promise.all(items.map(async (item) => {
      try {
        const response = await api<{ data: { id: string } }>("/media", { method: "POST", body: item.file, headers: { "Content-Type": item.file.type, "X-Project-ID": project.id, "X-File-Name": item.file.name } });
  ...
  async function resolveCandidate(candidate: Candidate, action: "confirm" | "discard") {
    try {
      const response = await api<{ data: { candidate: Candidate } }>(`/candidates/${candidate.id}/${action}`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() } });
  ...
  return (
    <div className="app-shell">
      ...
      <main className="workspace">
        ...
        {step === "interview" && project && session && <InterviewView project={project} session={session} answer={answer} setAnswer={setAnswer} uploads={uploads} busy={busy} error={messageError} onFiles={uploadFiles} onRetry={retryUpload} onRemove={(id) => setUploads((items) => items.filter((item) => item.id !== id))} onSend={() => void sendAnswer()} onSkip={() => void sendAnswer(true)} onFinish={() => setFinishOpen(true)} />}
        {step === "candidates" && project && <CandidateView project={project} candidates={candidates} processed={processedCandidates} error={messageError} onResolve={resolveCandidate} />}
      </main>
...
function SetupView({ form, setForm, busy, error, onSubmit }: { form: { brand_name: string; industry: string; custom_industry: string; core_product: string; origin: string; category: string; consent: boolean }; setForm: (value: typeof form) => void; busy: boolean; error: string | null; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
