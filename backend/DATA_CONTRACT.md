...
`tasks` 保存 `input_snapshot`、进度、尝试次数、幂等键、错误码和结果。路线以冻结输入生成；live 文字模型超时、限流、鉴权或返回结构异常时，路线任务会自动改用同一冻结事实生成三条可选基础路线，并在结果中记录 `mode=fallback` 与错误码，而不会把定调流程置为失败。选择路线后同步创建手册骨架，重新选择会创建新版本。手册同时保存原始生成快照、当前编辑内容和不可变版本，切换路线不会覆盖旧版本或用户修改。
