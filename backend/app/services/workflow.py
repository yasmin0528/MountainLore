...
def _generate_manual(task: dict[str, Any]) -> dict[str, Any]:
    ...
    resume_version_id = previous.get("manual_version_id")
    _task_update(task["id"], progress=20)
    if resume_version_id:
        with connect() as connection:
            existing_version = row_dict(connection.execute(
                "SELECT * FROM manual_versions WHERE id = ? AND project_id = ?", (resume_version_id, task["project_id"])
            ).fetchone())
        if not existing_version:
            raise ProviderError("manual_not_found", "无法恢复上次手册版本")
        version_id = resume_version_id
        content = json.loads(existing_version["content_json"])
        mode = "resume_assets"
    else:
        if provider.live:
            generated = provider.chat_json(
                model=settings.openai_next_text_model,
                instruction=(
                    "生成完整、可编辑的品牌视觉手册工作稿。只输出JSON，包含 brand_name,brand_introduction,slogan,"
                    "brand_strategy{audience,scenarios,emotion,altruism},story_system{main_story,chapters},"
                    "voice{do,dont},selling_points(保留claimIds),evidence_gaps,visual_system{keywords,logo_direction,packaging,pattern},applications,disclaimer。"
                    "不得新增无法由输入事实支持的功效或产地承诺。"
                ),
                context=snapshot,
            )
        ...
        with connect() as connection:
            version = connection.execute(
                "SELECT COALESCE(MAX(version), 0) + 1 FROM manual_versions WHERE project_id = ?", (task["project_id"],)
            ).fetchone()[0]
            version_id = new_id()
            timestamp = now()
            connection.execute(
                """INSERT INTO manual_versions
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, 'generating_assets', ?, ?)""",
                (version_id, task["project_id"], snapshot["direction"]["id"], version, json_value(content), json_value(content), timestamp, timestamp),
            )
            manual = row_dict(connection.execute("SELECT * FROM brand_manuals WHERE project_id = ?", (task["project_id"],)).fetchone())
            if manual:
                connection.execute(
                    ...
                    (snapshot["direction"]["id"], json_value(content), version_id, json_value(content), timestamp, task["project_id"]),
                )
            else:
                connection.execute(
    ...
    with connect() as connection:
        existing_assets = [dict(row) for row in connection.execute(
            "SELECT kind, media_asset_id FROM manual_assets WHERE manual_version_id = ? AND media_asset_id IS NOT NULL", (version_id,)
        )]
    existing_kinds = {item["kind"] for item in existing_assets}
    ...
    if provider.live:
        ...
        for offset, (kind, prompt) in enumerate(missing_prompts, start=1):
            try:
                asset_ids.append(_save_image_asset(task["project_id"], version_id, kind, provider.generate_image(prompt), prompt))
            except ProviderError as exc:
                image_errors.append({"kind": kind, "code": exc.code, "message": str(exc)})
            _task_update(task["id"], progress=55 + offset * 12)
    else:
        image_errors = [{"kind": kind, "code": "demo_mode", "message": "演示模式未生成图片"} for kind in prompts if kind not in existing_kinds]
    final_version_status = "ready" if len(existing_kinds | (set(prompts) - {item["kind"] for item in image_errors})) == 3 else "text_ready"
    with connect() as connection:
        connection.execute("UPDATE manual_versions SET status = ?, updated_at = ? WHERE id = ?", (final_version_status, now(), version_id))
        connection.execute("UPDATE projects SET current_stage = 'positioning', status = 'manual_ready', updated_at = ? WHERE id = ?", (now(), task["project_id"]))
    return {"manual_version_id": version_id, "asset_ids": asset_ids, "image_errors": image_errors, "mode": mode, "_partial": bool(image_errors)}
