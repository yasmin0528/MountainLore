...
class CreditsProvider:
    ...
    def weekly_tide_ideas(self, sources: list[dict[str, Any]], holidays: list[dict[str, str]]) -> list[WeeklyTideIdea]:
        instruction = (
                "基于已验链来源的正文摘录，生成最多6条面向商家的通用新消费机会。"
                "新消费指年轻消费者、社交媒体、电商和新品牌带来的变化，例如情绪价值、悦己、兴趣社交、体验、国潮小众审美、健康生活、户外、宠物、咖啡茶饮、美妆个护和潮玩等；只选择输入来源真正支持的变化。"
                "theme 必须是12至24字、直接告诉商家现在能做什么的动作句，优先以“把、为、用、做、给、将、设计、推出、策划、拍、写、开、让、借、围绕”开头。"
                "例如“把中秋礼盒做成能分享的开箱仪式”，不要写“可溯源供应链叙事”“地方物产跨区域销售的轻量履约叙事”这类抽象概念标题。"
                "所有灵感必须是品类通用的：theme、content_motif、applicable_scene 均不得点名任何具体农产品、食品或饮料品类；来源只提供消费信号，不能把来源中的品类套给商家。"
                "content_motif 要用通俗语言说明消费者变化与商家做法；applicable_scene 要写清内容、产品体验或触点怎么落地。"
